...
class BookingServiceTests {
    ...
    @Test
    fun `saveBooking should save booking and update availability time slot`() = runTest {
        val booking = BookingDataProvider.createBooking(status = BookingStatus.CONFIRMED)
    ...
    @Test
    fun `saveBooking should save booking and not update availability time slot`() = runTest {
        val booking = BookingDataProvider.createBooking(status = BookingStatus.CONFIRMED)
    ...
    fun `findBookingsByUserId should return bookings`() = runTest {
        ...
        coEvery { userProfileRepository.findByUserId(userId) } returns
            UserProfileDataProvider.createUserProfile()
