...
class UserProfileControllerTests {
    ...
    @MockBean private lateinit var userProfileService: UserProfileService
    ...
    @Test
    fun `userProfile should return 404 when UserProfile not found`() = runTest {
        val userId = "nonexistentUserId"
        Mockito.`when`(userProfileService.userProfile(userId)).thenThrow(UserNotFoundException())

        webTestClient.get().uri("/api/v1/user_profile/$userId").exchange().expectStatus().isNotFound
    }

    @Test
    fun `updateUserProfile should return 200 when UserProfile is updated`() = runTest {
        val userId = "123456"
        val userProfile = UserProfileDataProvider.createUserProfile(userId = userId)
    ...
    @Test
    fun `updateUserProfile should return 404 when UserProfile not found`() = runTest {
        val userId = "123456"
        val userProfile = UserProfileDataProvider.createUserProfile(userId = userId)
