...
class ClassesControllerTests {
    ...
    @MockitoBean private lateinit var fitnessClassesService: FitnessClassesService

    @Test
    fun `should return all fitness classes by day of week`() = runTest {
        val fitnessClass = FitnessClassDataProvider.createClass()

        // Given
        `when`(fitnessClassesService.fitnessClassesByDayOfWeek("Monday"))
            .thenReturn(flowOf(fitnessClass))
    ...
    @Test
    fun `should save fitness class`() = runTest {
        val fitnessClass = FitnessClassDataProvider.createClass()
    ...
    @Test
    fun `should update fitness class`() = runTest {
        val fitnessClass = FitnessClassDataProvider.createClass()
    ...
    @Test
    fun `should delete fitness class by id`() = runTest {
        // Given
        `when`(fitnessClassesService.deleteFitnessClassById("1")).thenReturn(Unit)

        // When & Then
        webTestClient.delete().uri("/api/v1/fitness_class/1").exchange().expectStatus().isOk
    }
}
