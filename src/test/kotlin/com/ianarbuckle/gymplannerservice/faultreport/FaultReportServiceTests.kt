...
class FaultReportServiceTests {
    ...
    private val faultReportService: FaultReportService = FaultReportServiceImpl(faultReportRepository)

    @Test
    fun `should return all fault reports`() =
        runTest {
            val faultReports =
                listOf(
    ...
    @Test
    fun `test save report should throw exception if already exists`() =
        runTest {
            val faultReport = FaultReportDataProvider.createFaultReport()
            coEvery { faultReportRepository.findAll() } returns flowOf(faultReport)

            val result =
                assertThrows<FaultReportAlreadyExistsException> {
    ...
    @Test
    fun `should delete fault report by id`() =
        runTest {
            val faultReportId = "1"
