...
class CheckInControllerTests {
    ...
    @Test
    fun `should return 200 when check-in is valid`() =
        runTest {
    ...
    @Test
    fun `should return 404 when trainer is not found`() =
        runTest {
            val request = CheckInDataProvider.createCheckInRequest()
            Mockito
                .`when`(checkInService.checkIn("999", request.checkInTime))
    ...
    @Test
    fun `should return 400 when trainer is not scheduled`() =
        runTest {
            val request = CheckInDataProvider.createCheckInRequest()
            Mockito
                .`when`(checkInService.checkIn("1", request.checkInTime))
    ...
    @Test
    fun `should return 400 when trainer has already checked in today`() =
        runTest {
            val request = CheckInDataProvider.createCheckInRequest()
            Mockito
                .`when`(checkInService.checkIn("1", request.checkInTime))
    ...
    @Test
    fun `should return 200 with LATE status when trainer checks in after shift start`() =
        runTest {
            val lateCheckIn =
    ...
    @Test
    fun `should return 200 when check-out is valid`() =
        runTest {
    ...
    @Test
    fun `should return 404 on check-out when trainer is not found`() =
        runTest {
            val checkOutTime = LocalDateTime.of(2026, 4, 21, 17, 0)
            val request = CheckInDataProvider.createCheckOutRequest(checkOutTime = checkOutTime)
            Mockito
                .`when`(checkInService.checkOut("999", checkOutTime))
                .thenThrow(TrainerNotFoundException())
    ...
    @Test
    fun `should return 400 when trainer has not checked in`() =
        runTest {
            val checkOutTime = LocalDateTime.of(2026, 4, 21, 17, 0)
            val request = CheckInDataProvider.createCheckOutRequest(checkOutTime = checkOutTime)
            Mockito
                .`when`(checkInService.checkOut("1", checkOutTime))
    ...
    @Test
    fun `should return 400 when trainer has already checked out`() =
        runTest {
            val checkOutTime = LocalDateTime.of(2026, 4, 21, 17, 0)
            val request = CheckInDataProvider.createCheckOutRequest(checkOutTime = checkOutTime)
            Mockito
