...
class CheckInServiceTests {
    ...
    @Test
    fun `should update trainer availability status to AVAILABLE after check-in`() =
        runTest {
            val trainer = PersonalTrainerDataProvider.createPersonalTrainer()
            val checkInTime = LocalDateTime.of(2026, 4, 21, 9, 0)
            val expected = CheckInDataProvider.createCheckIn(checkInTime = checkInTime)
    ...
    @Test
    fun `should save check-out and set trainer status to UNAVAILABLE`() =
        runTest {
            val trainer =
    ...
    @Test
    fun `should throw TrainerNotCheckedInException when no check-in exists today`() =
        runTest {
            val trainer = PersonalTrainerDataProvider.createPersonalTrainer()
            val checkOutTime = LocalDateTime.of(2026, 4, 21, 17, 0)
